...
fn gather_quad(r: vec4<f32>, g: vec4<f32>, b: vec4<f32>) -> Quad {
  var o: Quad;
  o.l = fma(r, vec4<f32>(255.0), fma(g, vec4<f32>(255.0), b * 255.0));
...
fn encode(@builtin(global_invocation_id) gid_raw: vec3<u32>) {
  ...
  if (u32(base_xy.x) + 4u <= params.width && u32(base_xy.y) + 4u <= params.height) {
    ...
    if (gray) {
      ...
      // of qsum/sxp/syp): a third of the colour path's load-side ALU. The
      // luma is rounded: r·765 alone misses 3k by an f32 ulp for 87 of the
      // 256 values (the f16 module's conversion snaps it), and (r·255)·3 is
      // exact but fast-math may refold it into r·765.
      let l0 = round(r0 * 765.0);
      let l1 = round(r1 * 765.0);
      let l2 = round(r2 * 765.0);
      let l3 = round(r3 * 765.0);
      let rt0 = r0.z + r0.y;
      let rt1 = r1.z + r1.y;
      let rt2 = r2.z + r2.y;
      let rt3 = r3.z + r3.y;
      ...
      col[0] = vec4<f32>(l0.w, l0.x, l2.w, l2.x);
      col[1] = vec4<f32>(l0.z, l0.y, l2.z, l2.y);
      col[2] = vec4<f32>(l1.w, l1.x, l3.w, l3.x);
      col[3] = vec4<f32>(l1.z, l1.y, l3.z, l3.y);
    } else {
  ...
  if (gray) {
    ...
    let cz = vec4<f32>(col[0].zw, col[1].zw);
    let cx = vec4<f32>(col[2].xy, col[3].xy);
    sel = fit_flip(
      vec4<f32>(col[0].xy, col[1].xy),
      select(cz, cx, fb),
      vec4<f32>(col[2].zw, col[3].zw),
  ...
  if (sel.est <= planar_est) {
    ...
    var msb = 0u;
    {
      ...
      let neg = select(vec4<u32>(0u), bits, d < vec4<f32>(0.0));
