...
// swapped on the way out. This is the only variant; there is no f16 module
// (the estimates below are exact in f32 integer arithmetic and the shader is
// load-bound, so f16 buys nothing).
//
...
// 6.0 ms @2048² on Apple/metal-3, this one 0.26 ms at −0.06..−0.22 dB,
// with a 16-loads-only null shader at 0.14 ms):
//
...
fn encode(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let bsum0 = select(sum0a, sum0b, bflip == 1u);
  let bsum1 = select(sum1a, sum1b, bflip == 1u);
  let bsq0 = select(sq0a, sq0b, bflip == 1u);
  let bsq1 = select(sq1a, sq1b, bflip == 1u);
  let bdiff = sel.diff;
  ...
  if (best_est > GATE) {
    ...
    if (any(nb.codes0 != codes0) || any(nb.codes1 != codes1)) {
      ...
      var n1: vec3<f32>;
      if (bdiff) {
        n0 = extend5(nb.codes0);
        n1 = extend5(nb.codes1);
      } else {
