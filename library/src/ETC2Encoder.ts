...
//
// The shader emits ETC1 individual/differential blocks (full modifier-table
// search + one base refit) and ETC2 planar blocks; T and H modes are never
// emitted. See `etc2.wgsl` / `etc2_ref.ts` for the algorithm and layout.
// There is no f16 variant: the encode is integer-exact 0..255 arithmetic
// whose error sums overflow f16, so the f32 module is the only one.
//
