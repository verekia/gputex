...
//   • A MODE 4 (rotation) encoder was ALSO built, DECIDED per block from
//     the fused covariance (residual = trace − axisᵀCaxis, no dual
//     encode), validated bit-exact against hardware bc7-rgba-unorm
//     sampling, and REVERTED (2026-07, git history has the full kernel).
//     Quality was real: +2.5–2.9 dB on normal maps, +1.9–2.4 on packed
//     atlases, +3.2 on the colour card. The unfixable part is WARP
//     POISONING: any warp holding one mode-4 block executes both modes'
//     passes. Measured floor across three structures (separate passes
//     1.77×, one parametric shared loop 1.41× but +22% on pure mode-6
//     photo content from lost constant-shift codegen, per-mode
//     specialised loops 1.55×): normal maps ran 1.4–1.5×, and even 5–10%
//     SCATTERED coverage on photo colour poisoned 77–85% of workgroups
//     (+21–27%) — a residual-concentration gate cannot unpoison warps
//     because coverage would need to fall below ~2%. Viable only with
//     subgroup ballots (uniform warps skip the other mode), which the
//     comparison target does not enable. decodeBC7Mode4Block stays in
//     the reference, hardware-validated, for a subgroups-era revival.
