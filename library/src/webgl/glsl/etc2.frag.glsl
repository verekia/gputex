...
// One 2×2 quad at top-left p: per-texel luma (gather order), channel sums,
...
void main() {
  ...
  if (gray) {
    ...
    // the quantised, clamped corners: −2·θ·rhs + θᵀGθ.
    vec3 rB = sxp * 0.25;
    vec3 rC = syp * 0.25;
    vec3 rA = total - rB - rC;
    ...
    // residual minus the flip-independent terms: −Σ||s||²/8 + κ·(Σℓ)²/24.
